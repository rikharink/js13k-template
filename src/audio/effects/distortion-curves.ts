//FROM: https://raw.githubusercontent.com/vitaliy-bobrov/js-rocks/main/src/app/audio/effects/distortion-curves.ts
export const curves = {
  ...
  cubic(_amount: number, curve: Float32Array, n: number) {
    for (let i = 0, x; i < n; ++i) {
      x = (i * 2) / n - 1;
      curve[i] = 1.5 * x - 0.5 * Math.pow(x, 3);
    }
  },
  kicker(amount: number, curve: Float32Array, n: number) {
